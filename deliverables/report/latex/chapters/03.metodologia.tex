...
Nel capitolo viene presentata l’architettura generale del sistema. Successivamente, l'implementazione dei diversi moduli della pipeline in MATLAB viene analizzata in dettaglio, con il supporto rappresentativo del paziente \textit{BRATS\_218.nii.gz}.
...
La pipeline prevede una fase preliminare e facoltativa di analisi esplorativa (\textit{Exploratory Data Analysis}, EDA), finalizzata al confronto delle diverse sequenze MRI mediante visualizzazione delle immagini, miglioramento del contrasto, rendering tridimensionale del volume e analisi degli istogrammi di intensità.

In seguito, le immagini sono sottoposte a una fase di pre-processing che include l’estrazione della \textit{slice} più rilevante, la normalizzazione delle intensità, la riduzione del rumore con filtro mediano e la costruzione di rappresentazioni multimodali. Viene, inoltre, generata la \textit{seed map}, utile per individuare la regione iniziale da cui avviare la segmentazione.

Quest'ultima è implementata mediante una strategia a cascata che combina più tecniche~\cite{peter2008}. Inizialmente, viene applicata una sogliatura multilivello basata sul metodo di \textit{Otsu} per individuare le regioni di interesse. Successivamente, l'algoritmo \textit{Region Growing} espande tali regioni a partire dal seme e, infine, il \textit{Marker-Controlled Watershed} viene impiegato per migliorare la maschera di segmentazione ottenuta.
...
Infine, le prestazioni della pipeline sono valutate mediante un confronto tra le maschere predette e le corrispondenti \textit{ground truth} binarie, utilizzando metriche standard quali Dice Score, \textit{Precision} e Sensitivity.
...
All’interno della cartella principale del progetto sono presenti le sottocartelle relative alla gestione dei dati e dei risultati. La cartella \textit{/dataset} contiene i volumi MRI in formato NIfTI per tutti i pazienti considerati e le corrispondenti \textit{ground truth}. La cartella \textit{/results} è, invece, destinata al salvataggio dei risultati. Per ciascun paziente, la pipeline salva le figure relative alle sequenze originali, agli step di pre-processing, ai risultati intermedi della segmentazione, al post-processing e al confronto finale con la \textit{ground truth}.
...
Lo script definisce i percorsi delle cartelle contenenti i volumi MRI e le relative \textit{ground truth}.
...
Prima di avviare l’elaborazione, viene inizializzato un array di struct denominato \textit{metrics}, di dimensione pari al numero di pazienti. Ogni elemento della struttura contiene il nome del paziente e le metriche di valutazione associate alle cinque configurazioni sperimentali (\textit{FLAIR}, \textit{T1c}, \textit{T2}, \textit{Fusion2} e \textit{Fusion3}).

\paragraph{Elaborazione iterativa}
Mediante un ciclo \textit{for} che itera su tutti i pazienti del dataset, vengono definiti il nome del file corrente, i percorsi specifici dell’immagine e della \textit{ground truth} e la cartella di output dedicata al paziente.

Per ciascun paziente, lo script richiama in sequenza la funzione \textit{pre\_processing}, che restituisce le immagini preprocessate delle diverse sequenze, le fusioni multimodali, la maschera di \textit{ground truth} della \textit{slice} selezionata e la \textit{seed map} per l'avvio della segmentazione. Inoltre, con chiamate ripetute alla funzione \textit{segmentation}, si ottengono le maschere di segmentazione post-processate per ciascuna delle configurazioni. Successivamente, attraverso la funzione \textit{evaluation}, ciascuna maschera predetta viene confrontata con la \textit{ground truth} e vengono calcolate le metriche di valutazione, poi salvate nella struttura \textit{metrics} aggiornando i campi relativi al paziente corrente.

\paragraph{Generazione dei risultati}
Per ciascun paziente viene costruita una figura (\textit{comparison\_results.png}) composta da sei subplot disposti su una griglia \(2 \times 3\). Nel primo pannello viene visualizzata la \textit{ground truth} con immagine \textit{T1} di sfondo, mentre nei cinque pannelli successivi vengono mostrate le segmentazioni ottenute dalle diverse configurazioni con il contorno della \textit{ground truth} in verde e quello della segmentazione in rosso.
...
Inizialmente, viene selezionato il paziente desiderato e vengono definiti i percorsi del volume MRI e della \textit{ground truth} in formato NIfTI. Inoltre, viene creata la cartella per il salvataggio delle figure prodotte durante l’analisi. Successivamente, vengono letti i volumi con la funzione \textit{niftiread} e convertiti in formato \textit{double}.

L'analisi è effettuata in modo semplificato sulla \textit{slice} centrale del volume, la numero 78. Nella pipeline finale, invece, la selezione della \textit{slice} avviene in modo automatico, individuando quella con maggiore estensione della regione tumorale.
...
\begin{figure}[H]
    ...
    \caption{Analisi delle sequenze MRI originali per le immagini \textit{FLAIR}, \textit{T1}, \textit{T1c} e \textit{T2} estratte dalla \textit{slice} selezionata e i relativi istogrammi delle intensità nella regione cerebrale.}
...
All'inizio, vengono caricati il volume MRI e la relativa \textit{ground truth} tramite la funzione \textit{niftiread}, poi convertiti in formato \textit{double}. A differenza di quanto descritto per la fase di EDA, viene individuata automaticamente la \textit{slice} più significativa calcolando, per ognuna di esse, il numero di pixel appartenenti alla regione tumorale nella \textit{ground truth} e selezionando quella che presenta la massima estensione della lesione.

Detta $G(x,y,z)$ la \textit{ground truth} binaria tridimensionale, lo score associato alla \textit{slice} $z$ è definito come:
\[
s(z) = \sum_{x}\sum_{y} \mathbf{1}\big(G(x,y,z)>0\big)
\]
La \textit{slice} scelta corrisponde quindi a:
\[
z_{\text{best}} = \arg\max_{z} s(z)
\]

Identificata tale \textit{slice}, da essa vengono estratte le sequenze \textit{FLAIR}, \textit{T1}, \textit{T1c}, \textit{T2} e le maschere di \textit{ground truth} corrispondenti.
...
Il codice genera una figura contenente le sequenze grezze e i relativi istogrammi delle intensità nella regione cerebrale per la sola \textit{slice} ottimale individuata, come riportato in Figura~\ref{fig:pre_raw_histogram}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/218_Raw_histogram.png}
    \caption{Sequenze MRI grezze estratte dalla \textit{slice} ottimale, con i relativi istogrammi delle intensità calcolati nella ROI cerebrale.}
...
\begin{figure}[H]
    ...
    \caption{\textit{Seed map} ottenuta applicando una sfocatura gaussiana alla rappresentazione \textit{Fusion3}.}
...
Prima di avviare la segmentazione, si individua automaticamente il seme iniziale sfruttando la \textit{seed map} generata nella fase di pre-processing.
...
\end{cases}
\]
dove $T_2$ rappresenta la soglia più alta restituita da \textit{Otsu}.

\paragraph{Region Growing con seme automatico}
A partire dal seme selezionato, viene applicato un algoritmo di \textit{Region Growing} mediante la funzione \textit{region\_growing.m}. Tale tecnica segmenta l’immagine espandendo iterativamente una regione iniziale verso i pixel adiacenti che soddisfano un opportuno criterio di omogeneità~\cite{adams1994}.

Un punto viene aggiunto alla regione se la sua intensità supera una determinata soglia calcolata a partire dalla maschera di \textit{Otsu}.
...
Dunque, si considerano tutte le intensità dei pixel nella maschera, si ordinano in senso crescente e si seleziona il valore al di sotto del quale cade il 6\% dei campioni. L’utilizzo di un percentile rende il metodo dinamico rispetto alla distribuzione delle intensità del singolo paziente. Il valore corrispondente è individuato empiricamente, confrontando i risultati di \textit{Dice Score} al variare del percentile, come illustrato in Figura~\ref{fig:percentile_dice}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images/percentile_dice.png}
    \caption{Andamento del \textit{Dice Score} medio al variare del percentile utilizzato per definire la soglia del \textit{Region Growing}.}
...
La maschera prodotta dal \textit{Region Growing} viene migliorata mediante l’algoritmo \textit{Marker-Controlled Watershed}~\cite{vincent1991}, che considera l’immagine come una superficie topografica e separa le regioni in base alle zone che si sviluppano attorno agli stessi minimi locali. 

Tuttavia, può soffrire di \textit{over-segmentation}, generando un numero elevato di regioni piccole e sparse. Per ridurre questo effetto, sono stati impiegati marcatori espliciti estratti dal \textit{Region Growing}~\cite{grau2004}.
...
\]
dove $\operatorname{bwdist}$ rappresenta la distanza euclidea e $\neg M_{\text{RG}}$ indica i pixel esterni alla maschera. Successivamente, i minimi locali della superficie vengono imposti utilizzando la maschera di \textit{Otsu} come marker:
\[
...
Per ciascuna configurazione analizzata, viene salvata una figura che riassume i passaggi della pipeline, tra cui la maschera ottenuta con \textit{Otsu} (in giallo), l'espansione del \textit{Region Growing} (in ciano), il risultato del \textit{Watershed} (in magenta) e la maschera finale non rifinita (in rosso), come riportato di seguito.
...
Nella Figura~\ref{fig:seg_flair}, la segmentazione iniziale ottenuta con \textit{Otsu} risulta ampia e con regioni spurie lungo la periferia cerebrale. Il \textit{Region Growing} consente di selezionare la componente connessa più pertinente attorno al seme, mentre il \textit{Marker-Controlled Watershed} ne rifinisce il contorno.
...
Nel caso della sequenza \textit{T1c} in Figura~\ref{fig:seg_t1c}, la sogliatura di \textit{Otsu} individua diverse regioni ad alta intensità distribuite nell’immagine, che non risultano sufficientemente connesse con l’intera estensione della lesione. Di conseguenza, le successive fasi di \textit{Region Growing} e \textit{Marker-Controlled Watershed} non riescono a produrre una segmentazione significativa.
...
Per la sequenza \textit{T2} riportata in Figura~\ref{fig:seg_t2}, la sogliatura di \textit{Otsu} riesce a individuare la regione tumorale, ma considera alcune aree secondarie errate.
...
Nel caso della rappresentazione bimodale in Figura~\ref{fig:seg_fusion2}, la sogliatura di \textit{Otsu} opera come nel caso precedente, sebbene in misura più contenuta rispetto alla sequenza \textit{FLAIR}.
...
Nel caso della rappresentazione multimodale in Figura~\ref{fig:seg_fusion3}, la sogliatura iniziale di \textit{Otsu} risulta già più selettiva rispetto alle altre configurazioni, concentrandosi maggiormente sulla regione tumorale rispetto alle zone periferiche.
...
L’ultima fase della pipeline riguarda la valutazione delle maschere di segmentazione ottenute. Tale operazione è implementata nella funzione \textit{evaluation.m}, che confronta la sovrapposizione della maschera binaria predetta con la corrispondente \textit{ground truth} binaria.
...
La funzione riceve in ingresso due maschere binarie, corrispondenti alla segmentazione predetta dalla pipeline (\textit{mask\_pred}) e alla \textit{ground truth} di riferimento (\textit{mask\_gt}). Entrambe le maschere vengono interpretate come immagini binarie della stessa dimensione, in cui i pixel appartenenti alla regione tumorale assumono valore logico vero, mentre i pixel di sfondo assumono valore falso.
...
\begin{itemize}
    ...
    \item \textbf{False Positive (FP)}: pixel classificati come tumore dalla predizione, ma assenti nella \textit{ground truth};
    \item \textbf{False Negative (FN)}: pixel appartenenti alla \textit{ground truth} tumorale, ma non rilevati dalla segmentazione.
\end{itemize}
...
La prima metrica calcolata è il \textit{Dice Score}, che rappresenta una misura di similarità spaziale tra la segmentazione predetta e la \textit{ground truth}. Esso è definito come:
\[
Dice = \frac{2TP}{2TP + FP + FN}.
\]

Il \textit{Dice Score} assume valori compresi tra 0 e 1. Un valore pari a 1 indica una sovrapposizione perfetta tra maschera predetta e \textit{ground truth}, mentre un valore pari a 0 indica assenza totale di sovrapposizione.

\paragraph{Sensitivity}
La seconda metrica è la \textit{Sensitivity}, nota anche come \textit{Recall}, che misura la capacità dell’algoritmo di individuare correttamente i pixel tumorali realmente presenti nella \textit{ground truth}:
\[
